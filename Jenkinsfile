...
pipeline{
    ...
    environment
    {
        ...
        IMAGE_NAME = 'schbros/finalproject1'
 ...
 stage("Check Docker Availability") {
    ...
    steps {
        script {
            ...
            if (!fileExists("${WORKSPACE}/zap_report.json")) {
                echo "⚠️ ZAP reports NOT FOUND — ZAP probably failed to write to the workspace."
            }
        }
    }
    post {
        always {
            archiveArtifacts artifacts: 'zap_report.json,zap_report.html', allowEmptyArchive: true
        }
    }
}
